    ...
    jm.layout_provider.prototype={
        ...
        _layout_offset_subnodes:function(nodes){
            ...
            while(i--){
                ...
                layout_data.offset_x = this.opts.hspace * layout_data.direction + pd.view.width * (pd.layout.direction + layout_data.direction);
    ...
    jm.view_provider.prototype={
        ...
        _center_root:function(){
            ...
            if(this.size.h > outer_h){
                ...
                // this.e_panel.scrollTop = (this.size.h - outer_h);
        ...
        show_nodes:function(){
            ...
            for(var nodeid in nodes){
                ...
                // expander.style.backgroundImage= 'url(images/icons/pack.png) center';
                ...
                if(node.isroot){
                    node_element.style.left = (_offset.x+p.x) - 80 + 'px';
                }
                if(!node.isroot && node.children.length>0){
                    expander_text = node.expanded?'':'';
                    expander.style.background = node.expanded  ? 'url(images/icons/spread.png) center':'url(images/icons/pack.png) center';
                    expander.style.backgroundSize = node.expanded  ? '20px':'20px';
                    p_expander= this.layout.get_expander_point(node);
                    expander.style.left = (_offset.x + p_expander.x) - 5 + 'px';
                    expander.style.top = (_offset.y + p_expander.y) - 2 + 'px';
